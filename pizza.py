...
def Order():
    def ordernow():
        ...
        try:
            ...
            tkinter.messagebox.showerror("Error", "Please enter valid ceredentials")
            order.destroy()
    ...
    def cancel_now():
        if (enter_order.get() == "" or enter_name.get() == ""):
            ...
            if (flag == 1):
                ...
                conn.commit()
                tkinter.messagebox.showinfo("Order Cancelled Succesfully", "Your Order has been cancelled")
        ...
        if (flag == 1):
            ...
            if (now.day > ttime.day):
                ...
                if ttime.day == now.day and ttime.hour == now.hour and now.minute <= ttime.minute + 20:
                    tkinter.messagebox.showinfo("Track Order",
                                          "Your order is Preparing\nIt will be delivered within 20 minutes of order")
                    track.destroy()
                else:
                    tkinter.messagebox.showinfo("Track Order", "Your order is Ready\nIt will be delivered soon")
                    track.destroy()
        else:
            tkinter.messagebox.showinfo("Track Order", "Please enter a valid Order ID")
